...
In this work we have applied HyperNEAT to the domain of Atari games. Many Atari games contain geometric regularities in the two-dimensional space of the game screen which allows HyperNEAT to quickly learn effective policies. To reduce the complexity of learning from the raw game screen, we introduce a game-independent visual processing hierarchy designed to identify classes of objects as well as the entity that the player controls on the game screen. Identified objects are provided as input to the HyperNEAT CPPN. Due to the computational overhead of visual processing each game screen, we utilize a parallel architecture to simultaneously evaluate multiple individuals. Results are presented for two Atari games, \textit{Freeway} and \textit{Asterix}. In both cases, HyperNEAT is shown to outperform previous benchmarks\cite{naddaf10} and in one case exceeds human level play. While none of the Atari games, if studied in isolation and given extensive feature engineering, likely presents too great a challenge to modern AI techniques, the full collection of over 900 Atari 2600 games presents a daunting task for a single learning agent to conquer. It is this goal of creating an agent capable of learning and seamlessly transitioning between many different tasks to which we strive.
